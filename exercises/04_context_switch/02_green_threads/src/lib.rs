...
impl Scheduler {
    ...
    pub fn spawn(&mut self, entry: extern "C" fn()) {
        ...
        let start_ptr = buf.as_ptr() as u64;
        let raw_stack_top = start_ptr + (STACK_SIZE as u64);
        let sp: u64 = (raw_stack_top - 16) & !15;
        let ra = thread_wrapper as *const () as u64;

        let thread = TaskContext {
            sp,
            ra,
            s0: 0,
            s1: 0,
    ...
    fn schedule_next(&mut self) {
        ...
        'out: {
            for i in (old_idx + 1)..self.threads.len() {
        ...
        unsafe {
            let ptr = self.threads.as_mut_ptr();

            let old_ctx = &mut (*ptr.add(old_idx)).ctx;
            let new_ctx = &(*ptr.add(next_idx)).ctx;
