...
impl Tlb {
    ...
    pub fn insert(&mut self, vpn: u64, ppn: u64, asid: u16, flags: u64) {
        ...
        let current_ptr = self.fifo_ptr;
        self.entries[current_ptr].valid = true;
        self.entries[current_ptr].vpn = vpn;
        self.entries[current_ptr].ppn = ppn;
        self.entries[current_ptr].asid = asid;
        self.entries[current_ptr].flags = flags;


        self.fifo_ptr = (current_ptr + 1) % self.capacity;
    ...
    pub fn translate(&mut self, vpn: u64) -> Option<u64> {
        ...
        for entry in &mut self.page_table {
            if entry.0 == self.current_asid && entry.1.vpn == vpn {
                self.tlb.insert(entry.1.vpn, entry.1.ppn, self.current_asid, entry.1.flags);
